// School coordinates - 7°00'25.16"S 110°30'50.90"E (DMS) converted to decimal degrees
export const SCHOOL_COORDS = {
    latitude: -7.006268,
    longitude: 110.514597
};

// Geofencing radius in meters 
export const SCHOOL_RADIUS = 120;

// Batas waktu hadir (jam WIB). Setelah jam ini, siswa tidak bisa presensi hadir.
export const HADIR_CUTOFF_HOUR = 9; // 09:00 WIB

// JWT Configuration
export const JWT_SECRET = process.env.JWT_SECRET || 'presensi-osis-mpk-secret-key-2024';
export const JWT_EXPIRES_IN = '24h';

// Admin credentials
export const ADMIN_CREDENTIALS = {
    kode: 'admin',
    sandi: '970408'
};

export const ATTENDANCE_STATUS = {
    HADIR: 'hadir',
    HADIR_LUAR_RADIUS: 'hadir_luar_radius',
    IZIN: 'izin',
    SAKIT: 'sakit',
    PULANG: 'pulang',
    TIDAK_HADIR: 'tidak_hadir'
};

// Organization types
export const ORGANIZATION_TYPES = {
    OSIS: 'OSIS',
    MPK: 'MPK'
};

// File paths for data storage
export const DATA_FILES = {
    SISWA: './data/siswa.xlsx',
    PRESENSI: './data/presensi.xlsx'
};
